...
/**
 * 스크린샷 → OCR → 경력·학력 자동 채우기 플로우 (upload → analyzing → failed → preview).
 * 바텀시트(HighlightLlmImportSheet)와 온보딩 단계(Step5CareerImport)에서 공용.
 * onDone 은 저장이 끝난 뒤 호출된다 (시트: 닫기 / 온보딩: 다음 단계).
 */
export function CareerScreenshotImportFlow({
  active,
  isPro,
  freeRemaining,
  onDone,
}: {
  /** 부모가 열려/보여진 상태일 때 true. false → 내부 상태 초기화 */
  active: boolean
  isPro: boolean
