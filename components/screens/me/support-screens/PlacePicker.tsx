...
// query 변경 시 debounce(300ms) 후 API 호출, category_group_code는 activeType에 따라 FD6 / CE7 사용
...
const TYPE_CONFIG: Record<PlaceType, { label: string; emoji: string; placeholder: string; emptyHint: string }> = {
  restaurant: { label: '맛집', emoji: '🍜', placeholder: '맛집 이름 또는 동네 검색...', emptyHint: '좋아하는 맛집을 검색해보세요' },
  cafe:       { label: '카페', emoji: '☕', placeholder: '카페 이름 또는 동네 검색...', emptyHint: '자주 가는 카페를 검색해보세요' },
}
...
export function PlacePicker({
  ...
  cafes: LifeMediaItem[]
  onChange: (restaurants: LifeMediaItem[], cafes: LifeMediaItem[]) => void
}) {
  const [activeType, setActiveType] = useState<PlaceType>('restaurant')
  const [query, setQuery] = useState('')

  const currentSelected = activeType === 'restaurant' ? restaurants : cafes

  const setCurrentSelected = (items: LifeMediaItem[]) => {
    if (activeType === 'restaurant') onChange(items, cafes)
    else onChange(restaurants, items)
  }

  const allSelected = [
    ...restaurants.map((r) => ({ ...r, placeType: 'restaurant' as PlaceType })),
    ...cafes.map((c) => ({ ...c, placeType: 'cafe' as PlaceType })),
  ]

  const selectedNames = new Set(currentSelected.map((s) => s.label))

  // TODO(backend): query 변경 시 debounce(300ms) 후 /api/places/search?q={query}&type={activeType} 호출
  ...
  const toggle = (place: MockPlace) => {
    if (selectedNames.has(place.name)) {
      setCurrentSelected(currentSelected.filter((s) => s.label !== place.name))
    } else {
      setCurrentSelected([...currentSelected, { label: place.name, sublabel: place.address }])
    }
  }

  const removeFromAll = (label: string, type: PlaceType) => {
    if (type === 'restaurant') onChange(restaurants.filter((r) => r.label !== label), cafes)
    else onChange(restaurants, cafes.filter((c) => c.label !== label))
  }
  ...
  return (
    <div>
        ...
        <div className="mb-4 space-y-2">
          {allSelected.map((item) => (
            ...
            >
              ...
              <button onClick={() => removeFromAll(item.label, item.placeType)} className="flex-shrink-0 p-1">
      ...
      <div className="flex gap-1.5 mb-3">
        {(['restaurant', 'cafe'] as PlaceType[]).map((type) => {
          const active = activeType === type
          const { label, emoji } = TYPE_CONFIG[type]
          return (
            <button
              key={type}
      ...
      {!query.trim() && (
        <p className="mt-3 text-center text-sm text-[var(--color-text-tertiary)]">{cfg.emptyHint}</p>
      )}
    </div>
  )
}
